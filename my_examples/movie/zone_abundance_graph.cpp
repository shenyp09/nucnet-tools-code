  ...
  my_graph_writer
{
  ...
  void operator()( std::ostream& out ) const
  {
    ...
    out <<
        boost::format( "node [shape=box, color=\"%s\", width=1, height=1, penwidth=3.];" ) % get_bounding_color()
        << std::endl;
...
)
{
  ...
  BOOST_FOREACH( nnt::Species species, species_list )
  {
    ...
    else
    {
      ...
      unsigned red = 0, green = 0x19, blue = 0x06;
      red = 255;
      green = (unsigned)(254 * (log10(d_y) / log10(d_cutoff)));
      blue = (unsigned)(254 * (log10(d_y) / log10(d_cutoff)));
      char hexcol[16];
